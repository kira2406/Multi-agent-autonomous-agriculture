...
class AgriEnv(ParallelEnv):
    def __init__(self, verbose = False):
        ...
        self.STATION1_IDX = 0
        self.STATION2_IDX = 2
        self.STATION3_IDX = 15
        self.MARKET_IDX = 3
        self.GARBAGE_IDX = 4
        self.SEEDSTN1_IDX = 5
        self.WATERTANK_IDX = 10
        ...
        self.plot_grids_indices = [12, 13, 14,17, 18, 19]

        
        self.observation_space = spaces.Tuple((
            spaces.Box(low=-1.0, high=2.0, shape=(len(self.plot_grids_indices)*6,), dtype=np.float32),  # Plot grid state
    ...
    def initialize_pygame(self):
        ...
        pygame.display.set_caption('4x5 Grid Environment')
    ...
    def ret_observation(self):
        ...
        for plot_state in self.plot_states:
            plot_state_obs.append([plot_state[0], plot_state[1], plot_state[2], plot_state[3], plot_state[5], plot_state[6]])
    ...
    def water_crops(self):
        ...
        if self.is_plot_grid(curr_cell):
            ...
            if self.plot_states[plot_id][0] == CropStages.NOT_PLANTED or self.plot_states[plot_id][2] == 1:
                ...
                if self.empty_plots == 0 and self.dry_plots == 0:
                    reward = 1.5
                return reward
    ...
    def step_seeder_agent(self, action):
        ...
        if action in seeder_agent_actions[:4]:
            if self.movement(self.seeder_agent, action):
                ...
                or self.is_station2(curr_cell) or self.is_station3(curr_cell) \
                or self.is_water_tank(curr_cell):
                    reward = -0.2
