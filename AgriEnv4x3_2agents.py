...
from constants import CropStages, CropTypes, CropInfo, GridElements
...
class AgriEnv(ParallelEnv):
    def __init__(self, verbose = False):
        ...
        self.action_space = {
            self.agents[0]: spaces.Discrete(8),
            self.agents[1]: spaces.Discrete(7)
        }
        ...
        self.observation_space = spaces.Tuple((
            spaces.Box(low=-1.0, high=2.0, shape=(8,), dtype=np.float32),  # Plot grid state
            spaces.Box(low=-1.0, high=3.0, shape=(2,), dtype=np.float32),  # Seeder agent state
            spaces.Box(low=-1.0, high=3.0, shape=(2,), dtype=np.float32)   # Harvester agent state
        ))
        ...
        self.grid_state[0] = GridElements.STATION1
        self.grid_state[1] = GridElements.STATION2
        self.grid_state[2] = GridElements.MARKET
        self.grid_state[3] = GridElements.GARBAGE

        self.plot_grids_indices = [10, 11]
        for plot_idx in self.plot_grids_indices:
            self.grid_state[plot_idx] = GridElements.PLOT

        self.grid_state[8] = GridElements.SEEDSTN1

        self.plot_grids = [[2,2], [2,3]]

        self.initialize_plots()

        self.seeder_agent.update_distance_target(self.manhatten_dist(self.seeder_agent.get_position(), 8)-1)

        self.harvester_agent.update_distance_target(self.manhatten_dist(self.harvester_agent.get_position(), 10)-1)
    ...
    def initialize_pygame(self):
        ...
        pygame.display.set_caption('3x4 Grid Environment')
    ...
    def initialize_environment(self):
        ...
        self.harvester_agent = HarvesterAgent(1)
    ...
    def ret_observation(self):
        ...
        for plot_state in self.plot_states:
            plot_state_obs.append([plot_state[0], plot_state[1], plot_state[3], plot_state[6]])
        return [np.array(plot_state_obs).flatten().tolist(), self.seeder_agent.state[:1]+self.seeder_agent.state[2:], self.harvester_agent.state[:1]+self.harvester_agent.state[2:]]
    ...
    def step(self, actions):
        ...
        step_reward = [self.seeder_agent.reward,self.harvester_agent.reward]
    ...
    def drop_seeds(self):
        ...
        reward = -0.30  # Severe penalty for mismatched seed type/station
        return reward
    ...
    def plant_seeds(self):
        """ Function to plant seeds into the plot grids based on the plot """
        ...
        if self.is_plot_grid(curr_cell):
            ...
            if self.plot_states[plot_id][0] == CropStages.NOT_PLANTED:
                ...
                self.empty_plots -= 1
                self.harvester_agent.reward += 0.5
                return 1
        return -0.3
    ...
    def step_seeder_agent(self, action):
        ...
        if action in seeder_agent_actions[:4]:
            if self.movement(self.seeder_agent, action):
                curr_cell = self.seeder_agent.get_position()
                if self.empty_plots == 0 and self.is_station1(curr_cell):
                    reward = 0.2
                elif self.empty_plots == 0 and self.is_plot_grid(curr_cell):
                    reward = -0.2
                elif self.is_market(curr_cell) or self.is_garbage(curr_cell):
                    reward = -0.2
                else:
                    reward = -0.01
        ...
        if self.empty_plots == 0:
            new_distance = self.manhatten_dist(self.seeder_agent.get_position(), 0)
        else:
            if self.seeder_agent.get_num_seeds() > 0:
                ...
                new_distance = self.manhatten_dist(self.seeder_agent.get_position(), 8)
    ...
    def step_harvester_agent(self, action):
        ...
        if action in harvester_agent_actions[:4]:
            if self.movement(self.harvester_agent, action):
                curr_cell = self.harvester_agent.get_position()
                if self.harvest_ready_plots == 0 and self.is_station2(curr_cell):
                    reward = 0.2
                elif self.is_station1(curr_cell) or self.is_seed_station_1(curr_cell) or self.is_seed_station_2(curr_cell) or self.is_seed_station_3(curr_cell):
                    reward = -0.2
                elif self.harvest_ready_plots == 0 and self.is_plot_grid(curr_cell):
                    reward = -0.5
                else:
                    reward = -0.01
            else:
            ...
            if self.is_station2(self.harvester_agent.get_position()) and self.harvest_ready_plots == 0:
                reward = 0.2
            else:
                reward = -0.2
        ...
        if self.harvest_ready_plots == 0 and self.harvester_agent.get_crops_type() == 0:
            new_distance = self.manhatten_dist(self.harvester_agent.get_position(), 1)
        else:
            if self.harvester_agent.get_crops_type() > 0:
                if self.harvester_agent.get_crops_value() > 0:
                    new_distance = self.manhatten_dist(self.harvester_agent.get_position(), 2)
                else:
                    new_distance = self.manhatten_dist(self.harvester_agent.get_position(), 3)
            else:
    ...
    def render(self):
        ...
        for i in range(GRID_HEIGHT*GRID_WIDTH):
            ...
            if self.grid_state[i] == GridElements.GRASS:
                ...
                img = PLOT_IMG
                if plot_state[0] == 0 and plot_state[1] == 0:
                    img = PLOT_IMG
                elif plot_state[0] == 0 and plot_state[1] == 1:
                ...
                elif plot_state[0] == 0 and plot_state[1] == 2:
                    img = PLOT_RICE_0_IMG
                    ...
                    img = PLOT_RICE_3_IMG
                elif plot_state[0] == 0 and plot_state[1] == 3:
                    img = PLOT_CORN_0_IMG
